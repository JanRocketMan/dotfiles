---
name: remote-session
description: Create a new GitLab issue and open a bidirectional communication channel for remote steering. Use when you want to name/describe the session upfront.
argument-hint: "<session description>"
user-invocable: true
allowed-tools: "Bash(glab *) Bash(echo *) Monitor Read Write(/tmp/remote-session.json)"
---

# Remote Session (create issue)

Create a new GitLab issue and open a bidirectional communication channel with the user. One issue per session.
...
1. Create a new issue for this session. Use `$ARGUMENTS` as the title:

       glab issue create -R "$GITLAB_REMOTE_PROJECT" --title "$ARGUMENTS" --description "Remote session. Post comments to send instructions." --no-editor
